...
import com.bj.requests.body.Part;
import com.bj.requests.body.RequestBody;
...
public final class RequestBuilder {
    ...
    public RequestBuilder forms(String params){
        String List[] = params.split("&");
        Map<String, Object> para = new HashMap<>();
        for(int i = 0;i<List.length;i++){
            para.put(List[i].substring(0,List[i].indexOf("=")),List[i].substring(List[i].indexOf("=")+1,List[i].length()));
        }
        return forms(para);
    }
